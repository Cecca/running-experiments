...
\myparagraph{Parameters.} We consider both 16-bit and 32-bit input representation, and inner product computation with a naive loop, an avx2 optimized loop, and an avx512 optimized loop. We use the SimHash filter described in Appendix~\ref{app:sketches} with failure probability $\delta \in \{0.001, 0.1, 0.2, 0.5, 0.7, 0.9, 0.99\}$. 
