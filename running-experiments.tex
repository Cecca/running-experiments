...
Previous work focused mainly on two areas: the design of experiments,
...
Structured text file formats like CSV address challenge (C4), but are expensive to parse and require to be fully loaded in main memory prior to the analysis, even when only a subset is needed.
...
% of the analysis, even when we are only interested in a subset of it.
% \footnote{
...
using schema migrations the database can evolve along with
...
has access only to the most recent results related to each algorithm/dataset: our code
